...
@registry.tokenizers("spacy_classla.PipelineAsTokenizer.v1")
def create_tokenizer(
    lang: str = "",
    dir: Optional[str] = None,
    package: str = "default",
    processors: Union[dict, str] = {},
    logging_level: Optional[Union[int, str]] = "DEBUG",
...
class ClasslaTokenizer(object):
    """Because we're only running the Classla pipeline once and don't split
    ...
    def __init__(self, snlp, vocab):
        ...
        snlp (classla.Pipeline): The initialized Classla pipeline.
    ...
    def __call__(self, text):
        """Convert a Classla Doc to a spaCy Doc.
        ...
        if not is_aligned or not all(ents):
            warnings.warn(
                ...
                f"valid tokens produced by the Classla tokenizer:\n"
                f"Words: {words}\n"
    ...
    def get_tokens_with_heads(self, snlp_doc):
        """Flatten the tokens in the Classla Doc and extract the token indices
        of the sentence start tokens to set is_sent_start.

        snlp_doc (classla.Document): The processed Classla doc.
    ...
    def token_vector(self, token):
        """Get Classla's pretrained word embedding for given token.

        token (Token): The token whose embedding will be returned
        RETURNS (np.ndarray[ndim=1, dtype='float32']): the embedding/vector.
            token.vector.size > 0 if Classla pipeline contains a processor with
